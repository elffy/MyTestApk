...
public class LocationManager {
    ...
    /**
     * should be called in workerThread.
     * @param loc
     * @return
     */
    public String getAddress(Location loc) {
        if (loc != null) {
            ...
            try {
                List<Address> addresses = geoCoder.getFromLocationName("凯德mall", 5);
                Log.d(TAG, "addresses.size: " + addresses.size());
                for (Address a : addresses) {
                    Log.d(TAG, "Address: " + a.toString());
                }
    ...
    private class LocationListener
        ...
        public void onLocationChanged(Location newLocation) {
            ...
            if (!mValid) {
                Log.d(TAG, mProvider + " Got first location.");
            }
